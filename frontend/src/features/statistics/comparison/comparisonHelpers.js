...
export function buildComparisonRows(positionProfile = 'Overall') {
  ...
  Object.entries(profileGroups).forEach(([category, statKeys]) => {
    ...
    statKeys.forEach((statKey) => {rows.push({ type: 'stat', id: `stat:${statKey}`, statKey, category })});
  });
...
export function getWinningSlotIdsForStat(statKey, selectedSlots) {
  const numericEntries = selectedSlots.map((slot) => ({ id: slot.id, value: toFiniteNumber(slot?.stats?.[statKey]) })).filter((entry) => entry.value !== null);
...
export function getWinningSlotIdsForWeeks(selectedSlots) {
  const numericEntries = selectedSlots.map((slot) => ({ id: slot.id, value: toFiniteNumber(slot?.weeksPlayed) })).filter((entry) => entry.value !== null);
...
export function buildComparisonWins(rows, selectedSlots) {
  ...
  rows.forEach((row) => {
    ...
    winners.forEach((slotId) => {wins[slotId] = (wins[slotId] || 0) + 1});
  });

  const weeksWinners = getWinningSlotIdsForWeeks(selectedSlots);
  weeksWinners.forEach((slotId) => {wins[slotId] = (wins[slotId] || 0) + 1});
