    ...
    <div className="charts-panel chart-wrapper">
      <ResponsiveContainer width="100%" height={420}>
        <ScatterChart margin={{ top: 12, right: 22, bottom: 38, left: 16 }}>
          <XAxis type="number" dataKey="avg_fp_ppr" name="Weekly Avg PPR" stroke="var(--color-text-muted)" tick={{ fontSize: 12 }} label={{ value: 'Weekly Average PPR', position: 'insideBottom', offset: -8 }}/>
          <YAxis type="number" dataKey="ceiling_fp_ppr" name="Weekly Ceiling PPR" stroke="var(--color-text-muted)" tick={{ fontSize: 12 }} label={{ value: 'Weekly Ceiling PPR', angle: -90, position: 'insideLeft' }}/>
          ...
          <Scatter data={data} fill="var(--color-primary)" stroke="var(--color-primary-dark)" onClick={(point) => {
