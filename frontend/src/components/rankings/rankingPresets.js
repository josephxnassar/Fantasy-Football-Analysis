...
function getPresetWeight(category, presetId) {
  ...
  if (presetId === 'volume') {
    if (name.includes('volume') || name.includes('workload') || name.includes('opportunity'))
      return 2;
  ...
  if (presetId === 'efficiency') {
    if (name.includes('efficiency') || name.includes('accuracy') || name.includes('separation'))
      return 2;
    if (name.includes('volume') || name.includes('workload') || name.includes('opportunity'))
      return 0;
    return 1;
  ...
  // upside
  if (name.includes('production') || name.includes('impact') || name.includes('yardage') || name.includes('bonus'))
    return 2;
  if (name.includes('efficiency') || name.includes('separation'))
    return 1;
  if (name.includes('risk'))
    return 0;
  return 1;
