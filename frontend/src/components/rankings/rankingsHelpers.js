...
export function buildRankings(players = [], rankableGroups, categoryWeights = {}, statWeights = {}, topN = 20) {
  ...
  for (const { stat } of weightedStats) {
    const values = players.map((player) => {
        ...
        if (value === null || !meetsStatThreshold(player, stat))
          return null;
        return value;
      }).filter((value) => value !== null);
    if (!values.length) 
      continue;
    statRanges.set(stat, {min: Math.min(...values), max: Math.max(...values), lowerIsBetter: isLowerBetterStat(stat)});
  }
  ...
  const ranked = players.map((player) => {
    ...
    }).filter(Boolean).sort((a, b) => b.score - a.score).slice(0, topN).map((player, index) => ({...player, rank: index + 1}));
