...
export function getStatOptions(position, statColumns = [], positionStatGroups) {
  ...
  if (!groups)
    return [];
  return Object.entries(groups).map(([category, stats]) => ({ category, stats: stats.filter((stat) => statColumns.includes(stat)) })).filter(({ stats }) => stats.length > 0)}

export function buildBarData(players = [], stat, topN) {
  if (!Array.isArray(players))
    return [];
  return players.filter((player) => player.stats[stat] != null && meetsStatThreshold(player, stat)).map((player) => ({ statKey: stat, name: player.name, position: player.position, team: player.team, headshot_url: player.headshot_url, value: player.stats[stat], statLabel: getStatLabel(stat) })).sort((a, b) => b.value - a.value).slice(0, topN)}
