...
export function usePlayerComparisonState() {
  ...
  const {comparisonSlots, selectionError, handlePlayerSelect, handleSeasonChange, handleRemovePlayer, resetForPositionProfile} = useComparisonSlots();
  ...
  const playerOptions = useMemo(() => {
    ...
    return players.map((player) => player.name).filter(Boolean).sort((a, b) => a.localeCompare(b));
  }, [chartData?.players]);
  const selectedPlayers = comparisonSlots.filter((slot) => Boolean(slot.playerName));
  const statWinnersByKey = useMemo(() => Object.fromEntries(comparisonRows.filter((row) => row.type === 'stat').map((row) => [row.statKey, getWinningSlotIdsForStat(row.statKey, selectedPlayers)])), [comparisonRows, selectedPlayers]);
