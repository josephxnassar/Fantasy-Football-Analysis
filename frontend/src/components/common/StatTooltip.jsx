...
export default function StatTooltip({ label, description, iconSize = 12 }) {
  ...
  // Reset cached position on close so reopen measures fresh placement.
  useEffect(() => {
    if (!open)
      setCoords(null);
  }, [open]);

  // Measure and position popover after render using real dimensions.
  useEffect(() => {
    ...
    if (top + popover.height > window.innerHeight - edgePadding)
  ...
  return (
    <span className="stat-tooltip-wrap">
      <button ref={triggerRef} type="button" className="stat-tooltip-trigger" aria-label={`Info about ${label}`} onClick={(e) => { e.stopPropagation(); setOpen((v) => !v); }}>
      ...
      {open && createPortal(
        <div ref={popoverRef} className="stat-tooltip-popover" role="tooltip" style={coords ? { top: coords.top, left: coords.left } : { top: 0, left: 0, visibility: 'hidden', pointerEvents: 'none' }}>
