...
describe('usePlayerDetails', () => {
  ...
  it('exposes error state when player load fails', async () => {
    getPlayer.mockRejectedValueOnce(new Error('Network error'));

    const { result } = renderHook(() => usePlayerDetails());

    await act(async () => {
      await result.current.handlePlayerClick('Unknown Player');
    });
  ...
  it('exposes error state when season change fails', async () => {
    getPlayer
      .mockResolvedValueOnce({
        data: {
          name: 'Patrick Mahomes',
      ...
      .mockRejectedValueOnce(new Error('Season error'));
    ...
    expect(result.current.detailsError).toBe('Failed to load season data');
    expect(result.current.loadingDetails).toBe(false);
  });
